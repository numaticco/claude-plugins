...
`agents/plan-applier.md`), not here. These briefs carry only what changes per run: paths and
...
Agent(numatic:plan-applier):
  ...
  prompt: |
    ...
    [The mechanical findings, VERBATIM - full text including every extend
     specification. Do not summarize; the applier's fidelity depends on the detail.]

    ## Excluded
    [List findings withheld as `needs human decision` or judged wrong, so the applier
...
Agent(general-purpose):
  ...
  prompt: |
    ...
    ## Sections the applier reports it edited
    [list from the applier's report]
    ...
    3. Did any edit contradict a section the applier did not touch?
